...
namespace dbtoaster {
  ...
  struct dV2_R_entry {
    int locn; int dateid; RingSum __av; dV2_R_entry* nxt; dV2_R_entry* prv;
  
    explicit dV2_R_entry() : nxt(nullptr), prv(nullptr) { }
    explicit dV2_R_entry(const int c0, const int c1, const RingSum& c2) { locn = c0; dateid = c1; __av = c2;  }
    ...
    FORCE_INLINE dV2_R_entry& modify(const int c0, const int c1) { locn = c0; dateid = c1;  return *this; }
    template<class Archive>
    void serialize(Archive& ar, const unsigned int version) const {
      ar << ELEM_SEPARATOR;
      DBT_SERIALIZATION_NVP(ar, locn);
