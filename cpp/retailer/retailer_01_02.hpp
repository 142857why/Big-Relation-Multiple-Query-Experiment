...
namespace dbtoaster {
  ...
  struct V1_R_entry {
    int locn; int dateid; int ksn; RingSum __av; V1_R_entry* nxt; V1_R_entry* prv;
  
    explicit V1_R_entry() : nxt(nullptr), prv(nullptr) { }
    explicit V1_R_entry(const int c0, const int c1, const int c2, const RingSum& c3) { locn = c0; dateid = c1; ksn = c2; __av = c3;  }
    ...
    FORCE_INLINE V1_R_entry& modify(const int c0, const int c1, const int c2) { locn = c0; dateid = c1; ksn = c2;  return *this; }
    template<class Archive>
    void serialize(Archive& ar, const unsigned int version) const {
      ar << ELEM_SEPARATOR;
      DBT_SERIALIZATION_NVP(ar, locn);
  ...
  struct dV2_R_entry {
    int locn; int dateid; int ksn; int8_t rain; int houseunits; RingSum __av; dV2_R_entry* nxt; dV2_R_entry* prv;
  
    explicit dV2_R_entry() : nxt(nullptr), prv(nullptr) { }
    explicit dV2_R_entry(const int c0, const int c1, const int c2, const int8_t c3, const int c4, const RingSum& c5) { locn = c0; dateid = c1; ksn = c2; rain = c3; houseunits = c4; __av = c5;  }
    dV2_R_entry(const dV2_R_entry& other) : locn(other.locn), dateid(other.dateid), ksn(other.ksn), rain(other.rain), houseunits(other.houseunits), __av(other.__av), nxt(nullptr), prv(nullptr) { }
    dV2_R_entry(const std::vector<std::string>& f, const RingSum& v) {
        /* if (f.size() < 5) return; */
        locn = std::stoi(f[0]); dateid = std::stoi(f[1]); ksn = std::stoi(f[2]); rain = (int8_t) std::stoi(f[3]); houseunits = std::stoi(f[4]); __av = v; nxt = nullptr; prv = nullptr;
    }
    
    FORCE_INLINE dV2_R_entry& modify(const int c0, const int c1, const int c2, const int8_t c3, const int c4) { locn = c0; dateid = c1; ksn = c2; rain = c3; houseunits = c4;  return *this; }
    template<class Archive>
    void serialize(Archive& ar, const unsigned int version) const {
      ar << ELEM_SEPARATOR;
      DBT_SERIALIZATION_NVP(ar, locn);
  ...
  struct dV2_R_mapkey01234_idxfn {
    ...
    FORCE_INLINE static bool equals(const dV2_R_entry& x, const dV2_R_entry& y) {
      return x.locn == y.locn && x.dateid == y.dateid && x.ksn == y.ksn && x.rain == y.rain && x.houseunits == y.houseunits;
    }
  };
  ...
  struct V2_R_entry {
    int locn; int dateid; int ksn; int8_t rain; int houseunits; RingSum __av; V2_R_entry* nxt; V2_R_entry* prv;
  
    explicit V2_R_entry() : nxt(nullptr), prv(nullptr) { }
    explicit V2_R_entry(const int c0, const int c1, const int c2, const int8_t c3, const int c4, const RingSum& c5) { locn = c0; dateid = c1; ksn = c2; rain = c3; houseunits = c4; __av = c5;  }
    V2_R_entry(const V2_R_entry& other) : locn(other.locn), dateid(other.dateid), ksn(other.ksn), rain(other.rain), houseunits(other.houseunits), __av(other.__av), nxt(nullptr), prv(nullptr) { }
    V2_R_entry(const std::vector<std::string>& f, const RingSum& v) {
        /* if (f.size() < 5) return; */
        locn = std::stoi(f[0]); dateid = std::stoi(f[1]); ksn = std::stoi(f[2]); rain = (int8_t) std::stoi(f[3]); houseunits = std::stoi(f[4]); __av = v; nxt = nullptr; prv = nullptr;
    }
    
    FORCE_INLINE V2_R_entry& modify(const int c0, const int c1, const int c2, const int8_t c3, const int c4) { locn = c0; dateid = c1; ksn = c2; rain = c3; houseunits = c4;  return *this; }
    template<class Archive>
    void serialize(Archive& ar, const unsigned int version) const {
      ar << ELEM_SEPARATOR;
      DBT_SERIALIZATION_NVP(ar, locn);
    ...
    void on_batch_update_R(const std::vector<DELTA_R_entry>::iterator &begin, const std::vector<DELTA_R_entry>::iterator &end) {
      ...
      { //foreach
        for (std::vector<DELTA_R_entry>::iterator e1 = begin; e1 != end; e1++) {
          ...
          long v1 = e1->__av;
          dV2_R.addOrDelOnZero(se1.modify(locn, dateid, ksn, rain, houseunits), (v1 * Ulift(inventoryunits)));
        }
      }
      ...
      { //foreach
        ...
        while (e2) {
          ...
          RingSum& v2 = e2->__av;
          V2_R.addOrDelOnZero(se2.modify(locn, dateid, ksn, rain, houseunits), v2);
          e2 = e2->nxt;
        }
      }
